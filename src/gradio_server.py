...
def export_progress_by_date_range(repo, days):
    ...
    return report, gr.update(value=report_file_path, visible=bool(report_file_path))  # 有报告后再展示下载组件

# 创建Gradio界面
demo = gr.Interface(
    fn=export_progress_by_date_range,  # 指定界面调用的函数
    title="GitHubSentinel",  # 设置界面标题
    flagging_mode="never",  # 隐藏 Flag 按钮
    inputs=[
        gr.Dropdown(
    ...
    outputs=[gr.Markdown(), gr.File(label="下载报告", visible=False)],  # 文件下载默认隐藏，生成后展示
)
